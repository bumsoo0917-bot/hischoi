...
export default function QuizApp(){
  ...
  if(screen==="quiz"&&current){
    return <main className="quiz-shell">
      ...
      <section className="question">
        ...
        <p className="concept">출제 영역 · {current.concept}</p><h1>{current.prompt}</h1>
  ...
  return <main><Nav />
    <section className="home-hero"><div><p className="eyebrow">큰별쌤 강의 기반 단계별 문제은행</p><h1>외우는 공부를 넘어,<br/><em>달인이 되는 수련</em></h1><p className="hero-desc">매번 달라지는 10문제를 풀고 9문제 이상 맞히세요. 네 단계를 통과하면 강의별 달인 칭호가 열립니다.</p><button className="primary" onClick={()=>goLesson(lessons[0])}>첫 수련 시작하기 →</button></div>
