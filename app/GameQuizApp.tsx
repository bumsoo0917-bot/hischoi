...
type RankingData={players:RankingPlayer[];me:(RankingPlayer&{defeated:Record<string,boolean>;collection:Record<string,boolean>})|null};
type Progress={
...
export default function GameQuizApp({user,signInHref,signOutHref}:{user:User|null;signInHref:string;signOutHref:string}){
  ...
  useEffect(()=>{
    ...
    fetch("/api/leaderboard",{cache:"no-store"})
      ...
      .then((data:RankingData)=>{
        if(!active)return;
        setRanking(data);
        if(data.me)setProgress(old=>({...old,gold:data.me!.currentGold,totalGold:data.me!.totalGold,defeated:data.me!.defeated,collection:data.me!.collection}));
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(()=>fetch("/api/leaderboard",{
      ...
      body:JSON.stringify({currentGold:progress.gold,totalGold:progress.totalGold,defeated:progress.defeated,collection:progress.collection}),
    }).catch(()=>{}),250);
    return()=>clearTimeout(timer);
  },[progress.gold,progress.totalGold,progress.defeated,progress.collection,ready,serverReady,user?.email]);
  ...
  if(screen==="result"&&encounter){
    ...
    return <main className="result-shell"><section className={`result ${practice||won?"passed":""}`}>
      <img className="result-art" src={encounter.image} alt={encounter.name}/>
  ...
  if(screen==="collection"){
    return <main>{nav}
      ...
      <section className="collection-section">{[1,2].map(lessonId=><div className="collection-group" key={lessonId}>
        <h2>{lessonId}강</h2>
        <div className="collection-grid">{encounters.filter(item=>item.lessonId===lessonId).map(item=>{
          ...
          return <article className={unlocked?"":"locked"} key={item.id}>
            {unlocked?<img src={item.image} alt={item.name}/>:<div className="locked-art">?</div>}
  ...
  if(screen==="leaderboard"){
    return <main>{nav}
      ...
      <section className="ranking-section">
        {user?<div className="my-ranking"><div><small>나의 순위</small><strong>{ranking.me?`${ranking.me.rank}위`:"기록 대기 중"}</strong><span>{user.displayName}</span></div><div><small>총 골드</small><strong>{ranking.me?.totalGold??progress.totalGold} G</strong></div><div><small>보스</small><strong>{ranking.me?.bossesDefeated??totalDefeated}명</strong></div><a href={signOutHref}>로그아웃</a></div>
          :<div className="signin-panel"><div><strong>로그인하면 기록이 계정에 저장됩니다.</strong><p>다른 기기에서도 골드와 보스 진행 상황을 이어갈 수 있습니다.</p></div><a href={signInHref}>ChatGPT로 로그인</a></div>}
        <div className="ranking-table"><div className="ranking-head"><span>순위</span><span>도전자</span><span>보스</span><span>총 골드</span><span>진행</span></div>
          {rankingLoading?<p className="ranking-empty">순위표를 불러오는 중...</p>:ranking.players.length===0?<p className="ranking-empty">아직 등록된 도전자가 없습니다.</p>:ranking.players.map(player=><div className="ranking-row" key={`${player.rank}-${player.displayName}`}><b>{player.rank}</b><strong>{player.displayName}</strong><span>{player.bossesDefeated}명</span><span>{player.totalGold} G</span><span className="mini-progress"><i style={{width:`${player.bossesDefeated/6*100}%`}}/><small>1강 {player.lesson1Bosses}/3 · 2강 {player.lesson2Bosses}/3</small></span></div>)}
        </div>
      </section>
    </main>;
  }
      ...
      <section className="stage-section">
        ...
        <div className="bosses">{bossLevels.map(item=>{
          ...
          return <article key={item} className={`${defeated?"complete":""} ${!unlocked?"locked":""}`}>
            <img className="boss-art" src={target.image} alt={target.name}/>
  ...
  return <main>{nav}
    <section className="home-hero"><div><p className="eyebrow">한국사 퀴즈 원정</p><h1>연습하고<br/><em>보스를 쓰러뜨리세요</em></h1><p className="hero-desc">5문제 연습으로 골드를 모으고, 각 강의의 보스 3명에게 도전하세요. 만난 유물과 유적은 도감에 기록됩니다.</p><button className="primary" onClick={()=>goLesson(lessons[0])}>첫 원정 시작 →</button></div>
      <div className="master"><div><span>현재 칭호</span><span className="gold-text">{progress.gold} G</span></div><strong>{rank}</strong><p>보스 {totalDefeated}/6 · 도감 {collectionCount}/{encounters.length}</p><button className="rank-button" onClick={openCollection}>도감 보기</button><button className="rank-button" onClick={openLeaderboard}>TOP 10 보기</button></div>
    </section>
    <section className="curriculum"><div className="section-head"><div><p className="eyebrow">30강 지도</p><h2>현재 1·2강을 플레이할 수 있습니다</h2></div></div>
...
function Nav({home,gold,user,onLeaderboard,onCollection,collectionCount}:{home?:()=>void;gold:number;user:User|null;onLeaderboard:()=>void;onCollection:()=>void;collectionCount:number}){
  return <nav><button className="brand" onClick={home}><span>史</span>한국사 수련장</button><span>기본별개념 · 30강 원정</span><button className="nav-rank" onClick={onCollection}>도감 {collectionCount}/10</button><button className="nav-rank" onClick={onLeaderboard}>TOP 10</button><b className="nav-gold">{gold} G</b>{user&&<small className="nav-user">{user.displayName}</small>}{home&&<button className="nav-link" onClick={home}>전체 강의</button>}</nav>;
}
