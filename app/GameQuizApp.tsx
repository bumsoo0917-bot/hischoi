...
export default function GameQuizApp({user,signOutHref,firebaseUser=false,anonymousUser=false}:{user:User|null;signOutHref:string;firebaseUser?:boolean;anonymousUser?:boolean}){
  ...
  return <main className="app-screen">{nav}
    ...
    <section className="curriculum"><div className="section-head"><div><p className="eyebrow">30강 원정 지도</p><h2>시대별로 강의를 선택하세요</h2></div><span className="section-period">{activeHomeEra.label} · {activeHomeEra.period}</span></div>
      ...
      <div className="lessons">{homeLessons.map(item=>{const done=defeatedCount(progress,item.id);return <button key={item.id} className={`${item.status} ${done===3?"complete":done>0?"in-progress":""}`} onClick={()=>goLesson(item)} disabled={item.status==="coming"} aria-label={`${item.id}강 ${item.shortTitle} · ${done===3?"달인 완료, 보스 3명 모두 처치":done>0?`진행 중, 보스 ${done}/3 처치`:item.status==="ready"?"미도전":"준비 중"}`}><span className="lesson-no">{String(item.id).padStart(2,"0")}</span><div><strong>{item.shortTitle}</strong><span className={`lesson-status ${done===3?"is-complete":""}`}>{done===3?"달인 완료 · 보스 3/3":done>0?`진행 중 · 보스 ${done}/3`:item.status==="ready"?"미도전":"준비 중"}</span></div><b className="lesson-card-icon" aria-hidden="true">{done===3?"✓":"→"}</b></button>})}</div>
    </section>
  </main>;
}
