...
const playableLessonIds=Array.from({length:20},(_,index)=>index+1);
...
export default function GameQuizApp({user,signOutHref,firebaseUser=false,anonymousUser=false}:{user:User|null;signOutHref:string;firebaseUser?:boolean;anonymousUser?:boolean}){
    ...
    return <main>{nav}
      ...
      <section className="ranking-section">
        ...
        <div className="ranking-table"><div className="ranking-head"><span>순위</span><span>도전자</span><span>보스</span><span>총 골드</span><span>진행</span></div>
          {rankingLoading?<p className="ranking-empty">순위표를 불러오는 중...</p>:ranking.players.length===0?<p className="ranking-empty">아직 등록된 도전자가 없습니다.</p>:ranking.players.map(player=><div className="ranking-row" key={`${player.rank}-${player.displayName}`}><b>{player.rank}</b><strong>{player.displayName}</strong><span>{player.bossesDefeated}명</span><span>{player.totalGold} G</span><span className="mini-progress"><i style={{width:`${player.bossesDefeated/60*100}%`}}/><small>완료 강의 {Object.values(player.lessonBosses??{}).filter(count=>count===3).length}/20</small></span></div>)}
        </div>
      </section>
    </main>;
  }
  ...
  return <main>{nav}
    <section className="home-hero"><div><p className="eyebrow">한국사 퀴즈 원정</p><h1>연습하고<br/><em>보스를 쓰러뜨리세요</em></h1><p className="hero-desc">5문제 연습으로 골드를 모으고, 각 강의의 보스 3명에게 도전하세요. 만난 유물과 유적은 도감에 기록됩니다.</p><button className="primary" onClick={()=>goLesson(lessons[0])}>첫 원정 시작 →</button></div>
      <div className="master"><div><span>현재 칭호</span><span className="gold-text">{progress.gold} G</span></div><strong>{rank}</strong><p>보스 {totalDefeated}/60 · 도감 {collectionCount}/{encounters.length}</p><button className="rank-button" onClick={openCollection}>도감 보기</button><button className="rank-button" onClick={openLeaderboard}>TOP 10 보기</button></div>
    </section>
    <section className="curriculum"><div className="section-head"><div><p className="eyebrow">30강 지도</p><h2>현재 1~20강을 플레이할 수 있습니다</h2></div></div>
