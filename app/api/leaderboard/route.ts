...
export async function GET(){
  try{
    ...
    const top=await db.select().from(playerProgress)
      ...
      .limit(10);
    let me:null|ReturnType<typeof publicPlayer>&{defeated:Record<string,boolean>}=null;
    if(user){
      ...
      if(row){
        ...
        me={...publicPlayer(row,better+1),defeated:JSON.parse(row.defeatedJson||"{}")};
      }
    }
...
export async function POST(request:Request){
  try{
    ...
    const payload=await request.json() as {currentGold?:unknown;totalGold?:unknown;defeated?:unknown};
    ...
    if(!Number.isInteger(currentGold)||currentGold<0||!Number.isInteger(totalGold)||totalGold<currentGold||!defeated){
    ...
    if(existing&&(totalGold<existing.totalGold||bossesDefeated<existing.bossesDefeated)){
