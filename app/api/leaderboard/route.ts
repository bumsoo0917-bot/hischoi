...
export async function GET(){
 try{const user=await getCurrentUser();const repository=progressRepository();const players=await repository.leaderboard(10);let me=null;
  if(user){const player=await repository.ensure(user.id,user.displayName);me={...publicPlayer(player,await repository.rankOf(player)),rewardVersion:player.rewardVersion,visualCorrect:player.visualCorrect,selectedTitle:player.selectedTitle,titles:player.titles,defeated:player.defeated,collection:player.collection,attempts:player.attempts,perfectBosses:player.perfectBosses,visualPerfectEras:player.visualPerfectEras};}
...
export async function POST(request:Request){
 try{const user=await getCurrentUser();if(!user)return Response.json({error:"로그인이 필요합니다."},{status:401});const input=parseProgressWrite(await request.json());if(!input)return Response.json({error:"올바르지 않은 진행 기록입니다."},{status:400});const player=await progressRepository().saveVerified(user.id,user.displayName,input);return Response.json({ok:true,progress:{currentGold:player.currentGold,totalGold:player.totalGold,xp:player.xp,level:player.level,rewardVersion:player.rewardVersion,visualCorrect:player.visualCorrect,title:player.title,selectedTitle:player.selectedTitle,titles:player.titles,defeated:player.defeated,collection:player.collection,attempts:player.attempts,perfectBosses:player.perfectBosses,visualPerfectEras:player.visualPerfectEras}});
